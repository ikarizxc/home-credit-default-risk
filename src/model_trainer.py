...
class ModelTrainer():
    ...
    def __init__(
        ...
        self._test_ids = []
        logger.info(f"ModelTrainer initialized, grid_search={self._use_grid_search}")
    ...
    def _set_model(
        ...
        if model_params == None:
            self._model = model
            logger.info("Set plain estimator without grid search")
            return
            
        self._grid_search = GridSearchCV(model, model_params, scoring='roc_auc', verbose=1)
        self._model = None
        logger.info("Configured GridSearchCV estimator")
    ...
    def _train_model(
        ...
        if self._use_grid_search:
            logger.info("Starting GridSearchCV fitting...")
            self._grid_search.fit(X_train, y_train)
            self._model = self._grid_search.best_estimator_
            logger.info(f"Best params: {self._grid_search.best_params_}")
        else:
            logger.info("Training estimator without grid search...")
        ...
        if hasattr(self._model, "predict_proba"):
            ...
            logger.error("Model has no method for probability/scores")
            raise AttributeError("Model has no method to produce scores for ROC-AUC")
        
        logger.info(f"ROC-AUC on validation data: {self._roc_auc_score:.4f}")
    ...
    def _train_val_split(
        ...
        logger.debug(f"Split data: train {X_train.shape}, val {X_val.shape}")
    ...
    def _save_submission(
        ...
        path = f'submissions/{file_name}.csv'
    ...
    def train_model(
        ...
        if drop_id:
            ...
            logger.debug("Dropped column SK_ID_CURR from features")
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        if hasattr(self._model, 'feature_importances_'):
            ...
            logger.error("Model has no attribute for feature importances")
            raise AttributeError(
                "Model doesn't have attribute to get importances"
            )
        features = getattr(self._model, 'feature_names_in_', None)
        df_imp = pd.DataFrame({
            'feature': features,
